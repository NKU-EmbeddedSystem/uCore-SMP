...
//            "pthread_rwlock_ebusy",
    ...
    // static test
    for (int i = 0; i < sizeof(static_test) / sizeof(char *); i++) {
        char *argv[] = {"runtest.exe", "-w", "entry-static.exe", static_test[i], NULL};
        test(argv);
    }

    // dynamic test
    for (int i = 0; i < sizeof(dynamic_test) / sizeof(char *); i++) {
        char *argv[] = {"runtest.exe", "-w", "entry-dynamic.exe", dynamic_test[i], NULL};
        test(argv);
    }
